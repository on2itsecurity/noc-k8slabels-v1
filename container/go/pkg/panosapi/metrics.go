...
func newPanCollector() *panCollector {
	return &panCollector{
		...
		panFailedAPICalls: prometheus.NewDesc(prometheus.BuildFQName(namespace, substring, "apicalls_failed_noresponse_count"),
			"Failed api calls to firewall without a response",
			[]string{"name", "path"}, nil,
		),
...
func (collector *panCollector) Collect(ch chan<- prometheus.Metric) {
	...
	// No reset as a counter should never reset https://prometheus.io/docs/concepts/metric_types/#counter
